<?php
	...
	$sql= "select ots.id_ot,
				...
				where 	ots.id_taller = talleres.id_taller and
						...
						ots.id_proceso = procesos.id_proceso";
	...
	if (mysql_num_rows($resulta) != 0){ //Si hay OT
		...
		while ($row = mysql_fetch_array($resulta)){ 
				if ($row['estado']==1){//Activo
					$jsondata['tabla'] .=   "<tr id='".$row['id_ot']."' style='cursor:pointer;' onclick='editar(".$row['id_ot'].")' data-toggle='modal' data-target='#edit_ot'>";
				}

				if ($row['estado']==3){//Cerrado
					$jsondata['tabla'] .=   "<tr id='".$row['id_ot']."' style='cursor:pointer;color:green;' onclick='editar(".$row['id_ot'].")' data-toggle='modal' data-target='#edit_ot'>";
				}

				if ($row['estado']==4){//Anulado
					$jsondata['tabla'] .=   "<tr id='".$row['id_ot']."' style='cursor:pointer;color:red;' onclick='editar(".$row['id_ot'].")' data-toggle='modal' data-target='#edit_ot'>";
				}
				   $jsondata['tabla'] .=   "<td>".$row['no_ot']."</td>
											...
											if ($row['estado']==3){//Cerrado
												$jsondata['tabla'] .= "<td>CERRADO</td>";
											}

											if ($row['estado']==4){//Anulado
												$jsondata['tabla'] .= "<td>ANULADO</td>";
											}																						
		...
		$jsondata['sql'] = $sql;
		$jsondata['fecha'] = $fec_actual;
		$jsondata['ot'] = $no_ot;


	} else { //No hay OT
		$jsondata['encontrado'] = "N";
		$jsondata['mensaje'] = "No existe Orden de Trabajo";
		$jsondata['sql'] = $sql;
		$jsondata['fecha'] = $fec_actual;
		$jsondata['ot'] = $no_ot;
	}
