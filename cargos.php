  ...
  <script type="text/javascript">
        ...
        function grabar(){
              ...
              if ($('#cargo_ed').val()==''){
                    ...
                    $("input[id^='id_tiphora_']").each(function() {
                        valor = $(this).val();
                        id = $(this).attr("id");
                        ...
                        tipo_id_h = id.substring(11, 12);

                        ids_tipo_hora.push(tipo_id_h);
                        precios_hora.push(valor);
                    });
              }

              var precxtipo = { 'id_hora': ids_tipo_hora, 'precio': precios_hora};
              var json_precxtipo = JSON.stringify(precxtipo);

              if(procede == 1){
                  $.post('g_act_cargo.php',
                    { id_cargo: $('#id_cargo_ed').val(),
                      cargo: $('#cargo_ed').val(),
                      precios: json_precxtipo },
                    function(datos) {  
                        
                      if(datos.trim() == "S"){
                          $("#mensaje_ed").html("Datos del Cargo Actualizados"); 
                          $('#mensaje_ed').addClass("ok");
                          $('#btn_grabar_cg').attr("disabled", true);                                   
                      }
        ...
        function grabar_new(){
              ...
              if(procede == 1){
                  $.post('g_new_cargo.php',
                    { cargo: $('#cargo_new').val(),
                      precios: json_precxtipo },
                    function(datos) {  
                        
                      if(datos.trim() == "S"){
                          $("#mensaje_new").html("Datos del Cargo Grabado"); 
        ...
        function borrar (cod_cargo) {
            var cargo = $("tr[id='"+cod_cargo+"']").children("td[id='cargo']").html();
            var id_cargo = cod_cargo;
            ...
            $.confirm({
                ...
                boxWidth: '40%',
                useBootstrap: false,
                content: '¿ Estas seguro de borrar el Cargo <b>' + cargo + '</b> ?',
                buttons: {
                    Si: {
                        text: 'Sí, Borrarlo', 
                        btnClass: 'btn-red',
                              ...
                              $.post('b_cargo.php',
                                ...
                                function(datos) {  
                                  if(datos.trim() == "S"){ // Usuario Borrado
                                      confirma = $.confirm({
                                                    title: 'Aviso',
                                                    content: 'El Cargo ha sido borrado!',
                                                    buttons: {
                                                      ok:{
                                                        btnClass: 'btn-red',
                                                        action: function () {
            ...
            if ($_SESSION["nivel_usuario"] == "A"){ 
               echo '<li class="treeview active">
                      ...
                      <ul class="treeview-menu">
                        ...
                        <li class="active"><a href="cargos.php"><i class="fa fa-circle-o"></i> Cargos y Precios</a></li>
      ...
      <div class="row">
          <div>
              <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2"> 
                <button type="button" class="btn btn-success" onclick="adicionar()" data-toggle="modal" data-target="#add_cargo"><span class="glyphicon glyphicon-plus-sign"></span> Adicionar Cargo</button>
              </div>
      ...
      <div class="row" style="margin-top: 10px">     
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12"> 
              <div>
                  <!-- Creacion de Tabla  -->
                  <?php
                      echo '<table id="tbl_cargos" class="table table-striped table-bordered .table-hover">
                                <thead>
                                  <tr bgcolor="#B9C3C5">
                                    <th>Cargo</th>';

                                    // Sacando los tipos de hora para los encabezados
                                    $sql = "select * from tipos_hora order by id_tipo_hora";
                                    $resulta = mysql_query($sql);

                                    if (mysql_num_rows($resulta) != 0){
                                        $no_columnas = mysql_num_rows($resulta);

                                        while ($row = mysql_fetch_array($resulta)){ 
                                              echo '<th id="'.$row['id_tipo_hora'].'" class="text-center">'.$row['tipo_hora'].'</th>';
                                        }
                                    }
                     ...
                     $sql = 'select cargos.id_cargo,
                         ...
                         where precio_xtiphora.id_cargo = cargos.id_cargo and
                               precio_xtiphora.id_tipo_hora = tipos_hora.id_tipo_hora 
                     ...
                     while ($row = mysql_fetch_array($query)){ 
                            ...
                            if ($var_id_cargo != $row['id_cargo']){
                                echo '<tr id="'.$row['id_cargo'].'">
                                        <td id="cargo">'.$row['cargo'].'</td>';

                                // Llevando cuenta de la Columna
                                $col_actual = 0;
                            }

                            $col_actual = $col_actual + 1;
                            $cadena.='<td class="text-center">'.$row['precio'].'</td>';
                            
                            $var_id_cargo = $row['id_cargo'];

                            if ($col_actual >= $no_columnas){
                                $col_actual = 0;
                                ...
                                echo '<button type="button" class="btn btn-success btn-sm" onclick="ver('.$row['id_cargo'].')" data-toggle="modal" data-target="#ver_cargo"><span class="glyphicon glyphicon-search"></span></button>';

                                echo '<button type="button" class="btn btn-warning btn-sm" onclick="editar('.$row['id_cargo'].')" data-toggle="modal" data-target="#edit_cargo"><span class="glyphicon glyphicon-edit"></span></button>';

                                echo '<button type="button" class="btn btn-danger btn-sm" onclick="borrar('.$row['id_cargo'].')"><span class="glyphicon glyphicon-remove"></span></button>';
...
<!--- Ventana Mostrar -->
<div id="ver_cargo" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>MOSTRAR CARGO</strong></h4>
      </div>
...
<!--- Ventana Editar -->
<div id="edit_cargo" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
            <button type="button" id="btn_grabar_cg" class="btn btn-success" onclick="grabar()">Grabar</button>
...
<div id="add_cargo" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>ADICIONAR CARGO</strong></h4>
      </div>
