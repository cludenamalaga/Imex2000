...
<head>
  ...
  <script type="text/javascript">
        ...
        function editar(cod_estado){
            $.post('edit_estado.php',
              { estado: cod_estado },
              function(datos) {   
        ...
        function grabar(){
              if ($('#estado_ed').val()==''){
                    ...
                    $('#estado_ed').focus();                        
                        ...
                        $.post('g_act_estado.php',
                          { id_estado: $('#id_estado_ed').val(),
                            estado: $('#estado_ed').val() },
                          function(datos) {  
                              
                            if(datos.trim() == "S"){
                                $("#mensaje_ed").html("Datos del Estado ha sido Actualizado"); 
                                $('#mensaje_ed').addClass("ok");
                                $('#btn_grabar_es').attr("disabled", true);                                   
                            }
        ...
        function grabar_new(){
              if ($('#estado_new').val()==''){
                    ...
                    $('#estado_new').focus();                        
                        ...
                        $.post('g_new_estado.php',
                          { estado: $('#estado_new').val() },
                          function(datos) {  
                              
                            if(datos.trim() == "S"){
                                $("#mensaje_new").html("Nuevo Estado ha sido Grabado!"); 
        ...
        function borrar (cod_estado) {
            var estado = $("tr[id='"+cod_estado+"']").children("td[id='estado']").html();
            var id_estado = cod_estado;
            ...
            $.confirm({
                ...
                boxWidth: '40%',
                useBootstrap: false,
                content: '¿ Estas seguro de borrar el Estado <b>' + estado + '</b> ?',
                buttons: {
                    Si: {
                        text: 'Sí, Borrarlo', 
                        btnClass: 'btn-red',
                              ...
                              $.post('b_estado.php',
                                ...
                                function(datos) {  
                                  if(datos.trim() == "S"){ // Usuario Borrado
                                      confirma = $.confirm({
                                                    title: 'Aviso',
                                                    content: 'El Estado ha sido borrado!',
                                                    buttons: {
                                                      ok:{
                                                        btnClass: 'btn-red',
                                                        action: function () {
            ...
            if ($_SESSION["nivel_usuario"] == "A"){ 
               echo '<li class="treeview active">
                      ...
                      <ul class="treeview-menu">
                        ...
                        <li class="active"><a href="estados.php"><i class="fa fa-circle-o"></i> Estados</a></li>
                      </ul>
                    </li>
      ...
      <div class="row">
          <div>
              <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2"> 
                <button type="button" class="btn btn-success" onclick="adicionar()" data-toggle="modal" data-target="#add_estado"><span class="glyphicon glyphicon-plus-sign"></span> Adicionar Estado</button>
              </div>
                    ...
                    <tbody>
                      <?php 
                         ...
                         while ($row = mysql_fetch_array($query)){ 
                              echo '<tr id="'.$row['id_estado'].'">';
                              echo '<td id="estado">'. $row['estado'] . '</td>';
                              echo '<td width=120>';
                              echo '<div class="btn-group">';

                              echo '<button type="button" class="btn btn-success btn-sm" onclick="ver('.$row['id_estado'].')" data-toggle="modal" data-target="#ver_estado"><span class="glyphicon glyphicon-search"></span></button>';

                              echo '<button type="button" class="btn btn-warning btn-sm" onclick="editar('.$row['id_estado'].')" data-toggle="modal" data-target="#edit_estado"><span class="glyphicon glyphicon-edit"></span></button>';

                              echo '<button type="button" class="btn btn-danger btn-sm" onclick="borrar('.$row['id_estado'].')"><span class="glyphicon glyphicon-remove"></span></button>';
...
<!--- Ventana Mostrar -->
<div id="ver_estado" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>MOSTRAR ESTADO</strong></h4>
      </div>
...
<!--- Ventana Editar -->
<div id="edit_estado" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>EDITAR ESTADO</strong></h4>
      </div>
      ...
      <div class="modal-footer">
            <button type="button" id="btn_grabar_es" class="btn btn-success" onclick="grabar()">Grabar</button>
...
<!--- Ventana Adicionar -->
<div id="add_estado" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>ADICIONAR ESTADO</strong></h4>
      </div>
