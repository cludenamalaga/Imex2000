...
<head>
  ...
  <script type="text/javascript">
        ...
        function ver(cod_material){
            $.post('det_material.php',
        ...
        function grabar(){
              if ($('#material_ed').val()==''){
                  ...
                  if ($('#precio_ed').val()==''){
                        ...
                        $.post('g_act_material.php',
                          { id_material: $('#id_material_ed').val(),
                            material: $('#material_ed').val(),
                            id_unidad: $('#cbo_unidades').val(),
                            precio: $('#precio_ed').val() },
                          function(datos) {  
                              
                            if(datos.trim() == "S"){
                                $("#mensaje_ed").html("Datos del Material ha sido Actualizado"); 
                                $('#mensaje_ed').addClass("ok");
                                $('#btn_grabar_ma').attr("disabled", true);                                   
                            }
        ...
        function grabar_new(){
              if ($('#material_new').val()==''){
                  ...
                  if ($('#cbo_unidades').val()=='X'){
                        $('#mensaje_new').html("Elija Unidad!");
                        $('#mensaje_new').addClass("error");
                        $('#cbo_unidades').focus();                            
                    }
                    else {
                      if ($('#precio_new').val()==''){
                            ...
                            $.post('g_new_material.php',
                              { material: $('#material_new').val(),
                                unidad: $('#cbo_unidades').val(),
                                precio: $('#precio_new').val() },
                              function(datos) {  
                                  
                                if(datos.trim() == "S"){
                                    $("#mensaje_new").html("Nuevo Material ha sido Grabado!"); 
        ...
        function borrar (cod_material) {
            var material = $("tr[id='"+cod_material+"']").children("td[id='material']").html();
            var id_material = cod_material;
            ...
            $.confirm({
                ...
                boxWidth: '40%',
                useBootstrap: false,
                content: '¿ Estas seguro de borrar el Material <b>' + material + '</b> ?',
                buttons: {
                    Si: {
                        text: 'Sí, Borrarlo', 
                        btnClass: 'btn-red',
                              ...
                              $.post('b_material.php',
                                ...
                                function(datos) {  
                                  if(datos.trim() == "S"){ // Usuario Borrado
                                      confirma = $.confirm({
                                                    title: 'Aviso',
                                                    content: 'El Material ha sido borrado!',
                                                    buttons: {
                                                      ok:{
                                                        btnClass: 'btn-red',
                                                        action: function () {
                                      ...
                                      confirma = $.confirm({
                                                    title: 'Aviso',
                                                    content: 'DENEGADO.. Existe Ordenes de ese Material REGISTRADOS!',
                                                    buttons: {
                                                      ok:{
                                                        btnClass: 'btn-red',
                                                        action: function () {
            ...
            if ($_SESSION["nivel_usuario"] == "A"){ 
               echo '<li class="treeview active">
                      ...
                      <ul class="treeview-menu">
                        ...
                        <li class="active"><a href="material.php"><i class="fa fa-circle-o"></i> Materiales</a></li>
      ...
      <div class="row">
          <div>
              <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2"> 
                <button type="button" class="btn btn-success" onclick="adicionar()" data-toggle="modal" data-target="#add_material"><span class="glyphicon glyphicon-plus-sign"></span> Adicionar Material</button>
              </div>
          ...
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12"> 
              <div>
                  <table id="tbl_unidades" class="table table-striped table-bordered .table-hover">
                    ...
                    <tbody>
                      <?php 
                         $sql = 'select materiales.id_material,
                                        materiales.material,
                                        unidades.id_unidad,
                         ...
                         while ($row = mysql_fetch_array($query)){ 
                              echo '<tr id="'.$row['id_material'].'">';
                              echo '<td id="material">'. $row['material'] . '</td>';
                              echo '<td>'. $row['simb_unidad'] . '</td>';
                              echo '<td>'. $row['precio'] . '</td>';
                              echo '<td width=120>';
                              echo '<div class="btn-group">';

                              echo '<button type="button" class="btn btn-success btn-sm" onclick="ver('.$row['id_material'].')" data-toggle="modal" data-target="#ver_material"><span class="glyphicon glyphicon-search"></span></button>';

                              echo '<button type="button" class="btn btn-warning btn-sm" onclick="editar('.$row['id_material'].')" data-toggle="modal" data-target="#edit_material"><span class="glyphicon glyphicon-edit"></span></button>';

                              echo '<button type="button" class="btn btn-danger btn-sm" onclick="borrar('.$row['id_material'].')"><span class="glyphicon glyphicon-remove"></span></button>';
...
<!--- Ventana Mostrar -->
<div id="ver_material" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>MOSTRAR MATERIAL</strong></h4>
      </div>
...
<!--- Ventana Editar -->
<div id="edit_material" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>EDITAR MATERIAL</strong></h4>
      </div>
      ...
      <div class="modal-footer">
            <button type="button" id="btn_grabar_ma" class="btn btn-success" onclick="grabar()">Grabar</button>
...
<!--- Ventana Adicionar -->
<div id="add_material" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>ADICIONAR MATERIAL</strong></h4>
      </div>
