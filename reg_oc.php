...
<head>
  ...
  <title>Otros Costos | IMEX 2000</title>
  ...
  <script type="text/javascript">
        ...
        function add_row_new(){
            if ($("#fec_tarea").val()==""){
                ...
                if ($("#no_ot_tarea").val()==""){
                  $.alert('Ingresar Número de OT!');
                  return;          

                } else {
                    if ($("#detalle").val()==""){
                        $.alert('Ingresar el Detalle!');
                        return;

                    } else {
                        if ($("#total").val()==""){
                            $.alert('Ingresar el Total!');
                            return;
                        } else {
                            $.post('val_addrow_n_oc.php',
                                  {   
                                      fecha: $("#fec_tarea").val(),
                                      ot: $("#no_ot_tarea").val(),
        ...
        function del_row(fila){
            $.confirm({
                ...
                boxWidth: '40%',
                useBootstrap: false,
                content: '¿ Borrar Registro del Costo ?',
                buttons: {
                    Si: {
                        text: 'Sí, Borrarlo', 
                        btnClass: 'btn-red',
        ...
        function editar_new(id_mobra){
            ...
            var detalle = $("#tbl_manobra").find("tr[id='"+id_mobra+"']").children("td[id='detalle']").html();

            var total = $("#tbl_manobra").find("tr[id='"+id_mobra+"']").children("td[id='total']").html();

            $.post('editar_oc_n.php',
              {   id_fila : id_mobra,
                  no_ot_n : no_ot,
                  fecha: fecha,
                  id_per: id_per,
        ...
        function grabar_ed_n(){
              ...
              if ($("#fecha_ed").val() == ""){
                  ...
                  if ($("#detalle_ed").val() == ""){
                      $("#mensaje_ed").html("Debe Ingresar Detalle");
                      $('#mensaje_ed').addClass("error");
                      $("#detalle_ed").focus();
                  } else {
                      if ($("#total_ed").val() == ""){
                          $("#mensaje_ed").html("Debe Ingresar Total");
                          $('#mensaje_ed').addClass("error");
                          $("#detalle_ed").focus();
                      } else {
                          console.log("id_fila->"+$("#id_fila_oc").val());

                          console.log('no_ot->'+$("#var_oc_no_ot").val());

                          console.log('fecha->'+$("#fecha_ed").val());
                              
                          console.log('id_per->'+$("#cbo_personal_ed").val());

                          console.log('detalle->'+$("#detalle_ed").val());

                          console.log('total->'+$("#total_ed").val());

                          $.post('g_editar_oc_n.php',
                            {   id_fila: $("#id_fila_oc").val(),
                                no_ot: $("#var_oc_no_ot").val(), 
                                fecha: $("#fecha_ed").val(),
                                id_per: $("#cbo_personal_ed").val(),
                                detalle: $("#detalle_ed").val(),
                                total: $("#total_ed").val()
                            },
                            function(datos) {  
        ...
        function grabando(){
              ...
              if (no_filas != 0){
                  $('#tbl_manobra tbody tr').each(function () {
                      var no_ot     = $(this).find("td[id='ot']").html();
                      var fecha     = $(this).find("td[id^='fecha_']").html();
                      var id_per    = $(this).find("td[id^='per_']").attr("id").substr(4);
                      var detalle   = $(this).find("td[id='detalle']").html();
                      var total     = $(this).find("td[id='total']").html();
        ...
        function cerrar(){
            $.post('cancelar_oc.php',
              function(datos) {  

             },"json");

             window.location="reg_oc.php";
        }
...
<div id="edit_mo" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>MODIFICAR OTRO COSTO</strong></h4>
      </div>
