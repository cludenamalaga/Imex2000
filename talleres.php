...
<head>
  ...
  <title>Talleres | IMEX 2000</title>
  ...
  <script type="text/javascript">
        ...
        function editar(cod_taller){
            $.post('edit_taller.php',
              { taller: cod_taller },
              function(datos) {   
        ...
        function grabar(){
              if ($('#taller_ed').val()==''){
                    ...
                    $('#taller_ed').focus();                        
                        ...
                        $.post('g_act_taller.php',
                          { id_taller: $('#id_taller_ed').val(),
                            taller: $('#taller_ed').val() },
                          function(datos) {  
                              
                            if(datos.trim() == "S"){
                                $("#mensaje_ed").html("Datos del Taller ha sido Actualizado"); 
                                $('#mensaje_ed').addClass("ok");
                                $('#btn_grabar_ta').attr("disabled", true);                                   
                            }
        ...
        function grabar_new(){
              if ($('#taller_new').val()==''){
                    ...
                    $('#taller_new').focus();                        
                        ...
                        $.post('g_new_taller.php',
                          { taller: $('#taller_new').val() },
                          function(datos) {  
                              
                            if(datos.trim() == "S"){
                                $("#mensaje_new").html("Nuevo Taller ha sido Grabado!"); 
        ...
        function borrar (cod_taller) {
            var taller = $("tr[id='"+cod_taller+"']").children("td[id='taller']").html();
            var id_taller = cod_taller;
            ...
            $.confirm({
                ...
                boxWidth: '40%',
                useBootstrap: false,
                content: '¿ Estas seguro de borrar el Taller <b>' + taller + '</b> ?',
                buttons: {
                    Si: {
                        text: 'Sí, Borrarlo', 
                        btnClass: 'btn-red',
                        action: function () {
                              $.post('b_taller.php',
                                { cod_taller: id_taller},
                                function(datos) {  
                                  if(datos.trim() == "S"){ // Usuario Borrado
                                      confirma = $.confirm({
                                                    title: 'Aviso',
                                                    content: 'El Taller ha sido borrado!',
                                                    buttons: {
                                                      ok:{
                                                        btnClass: 'btn-red',
                                                        action: function () {
            ...
            if ($_SESSION["nivel_usuario"] == "A"){ 
               echo '<li class="treeview active">
                      ...
                      <ul class="treeview-menu">
                        ...
                        <li class="active"><a href="talleres.php"><i class="fa fa-circle-o"></i> Talleres</a></li>
      ...
      <div class="row">
          <div>
              <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2"> 
                <button type="button" class="btn btn-success" onclick="adicionar()" data-toggle="modal" data-target="#add_taller"><span class="glyphicon glyphicon-plus-sign"></span> Adicionar Taller</button>
              </div>
          ...
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12"> 
              <div>
                  <table id="tbl_unidades" class="table table-striped table-bordered .table-hover">
                    ...
                    <tbody>
                      <?php 
                         $sql = 'select * from talleres order by taller asc';
                         $query = mysql_query($sql); 

                         while ($row = mysql_fetch_array($query)){ 
                              echo '<tr id="'.$row['id_taller'].'">';
                              echo '<td id="taller">'. $row['taller'] . '</td>';
                              echo '<td width=120>';
                              echo '<div class="btn-group">';

                              echo '<button type="button" class="btn btn-success btn-sm" onclick="ver('.$row['id_taller'].')" data-toggle="modal" data-target="#ver_taller"><span class="glyphicon glyphicon-search"></span></button>';

                              echo '<button type="button" class="btn btn-warning btn-sm" onclick="editar('.$row['id_taller'].')" data-toggle="modal" data-target="#edit_taller"><span class="glyphicon glyphicon-edit"></span></button>';

                              echo '<button type="button" class="btn btn-danger btn-sm" onclick="borrar('.$row['id_taller'].')"><span class="glyphicon glyphicon-remove"></span></button>';
...
<!--- Ventana Mostrar -->
<div id="ver_taller" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>MOSTRAR TALLER</strong></h4>
      </div>
...
<!--- Ventana Editar -->
<div id="edit_taller" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>EDITAR TALLER</strong></h4>
      </div>
      ...
      <div class="modal-footer">
            <button type="button" id="btn_grabar_ta" class="btn btn-success" onclick="grabar()">Grabar</button>
...
<!--- Ventana Adicionar -->
<div id="add_taller" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>ADICIONAR TALLER</strong></h4>
      </div>
