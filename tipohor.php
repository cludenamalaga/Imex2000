...
<head>
  ...
  <script type="text/javascript">
        ...
        function ver(cod_tipo){
            $.post('det_tipohor.php',
              { tipo: cod_tipo },
              function(datos) {   
        ...
        function grabar(){
              if ($('#tipo_ed').val()==''){
                  ...
                  if ($('#simbolo_ed').val()==''){
                        ...
                        $.post('g_act_tipohor.php',
                          { id_tipo: $('#id_tipo_ed').val(),
                            tipo: $('#tipo_ed').val(),
                            simbolo: $('#simbolo_ed').val() },
                          function(datos) {  
                              
                            if(datos.trim() == "S"){
                                $("#mensaje_ed").html("Datos del Tipo de Hora Actualizado"); 
                                $('#mensaje_ed').addClass("ok");
                                $('#btn_grabar_tipo').attr("disabled", true);                                   
                            }
                  ...
                  if ($('#simbolo_new').val()==''){
                        ...
                        $.post('g_new_tipo.php',
                          ...
                          function(datos) {  
                            ...
                            if(datos.trim() == "S"){
                                $("#mensaje_new").html("Nueva Tipo de Hora Grabado!"); 
        ...
        function borrar (cod_tipo) {
            var tipo = $("tr[id='"+cod_tipo+"']").children("td[id='tipohora']").html();
            var id_tipo = cod_tipo;
            ...
            $.confirm({
                ...
                boxWidth: '40%',
                useBootstrap: false,
                content: '¿ Estas seguro de borrar Tipo de Hora <b>' + tipo + '</b> ?',
                buttons: {
                    Si: {
                        text: 'Sí, Borrarlo', 
                        btnClass: 'btn-red',
                              ...
                              $.post('b_tipo.php',
                                ...
                                function(datos) {  
                                  if(datos.trim() == "S"){ // Usuario Borrado
                                      confirma = $.confirm({
                                                    title: 'Aviso',
                                                    content: 'El Tipo de Hora ha sido borrado!',
                                                    buttons: {
                                                      ok:{
                                                        btnClass: 'btn-red',
                                                        action: function () {
            ...
            if ($_SESSION["nivel_usuario"] == "A"){ 
               echo '<li class="treeview active">
                      ...
                      <ul class="treeview-menu">
                        ...
                        <li class="active"><a href="tipohor.php"><i class="fa fa-circle-o"></i> Tipos de Horas</a></li>
      ...
      <div class="row">
          <div>
              <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2"> 
                <button type="button" class="btn btn-success" onclick="adicionar()" data-toggle="modal" data-target="#add_tipo"><span class="glyphicon glyphicon-plus-sign"></span> Adicionar Tipo</button>
              </div>
          ...
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12"> 
              <div>
                  <table id="tbl_unidades" class="table table-striped table-bordered .table-hover">
                    ...
                    <tbody>
                      <?php 
                         $sql = 'select * from tipos_hora order by tipo_hora asc';
                         $query = mysql_query($sql); 

                         while ($row = mysql_fetch_array($query)){ 
                              echo '<tr id="'.$row['id_tipo_hora'].'">';
                              echo '<td id="tipohora">'. $row['tipo_hora'] . '</td>';
                              echo '<td>'. $row['simbolo'] . '</td>';
                              echo '<td width=120>';
                              echo '<div class="btn-group">';

                              echo '<button type="button" class="btn btn-success btn-sm" onclick="ver('.$row['id_tipo_hora'].')" data-toggle="modal" data-target="#ver_tipo"><span class="glyphicon glyphicon-search"></span></button>';

                              echo '<button type="button" class="btn btn-warning btn-sm" onclick="editar('.$row['id_tipo_hora'].')" data-toggle="modal" data-target="#edit_tipo"><span class="glyphicon glyphicon-edit"></span></button>';

                              echo '<button type="button" class="btn btn-danger btn-sm" onclick="borrar('.$row['id_tipo_hora'].')"><span class="glyphicon glyphicon-remove"></span></button>';
...
<!--- Ventana Mostrar -->
<div id="ver_tipo" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>MOSTRAR TIPO DE HORA</strong></h4>
      </div>
...
<!--- Ventana Editar -->
<div id="edit_tipo" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>EDITAR TIPO DE HORA</strong></h4>
      </div>
      ...
      <div class="modal-footer">
            <button type="button" id="btn_grabar_tipo" class="btn btn-success" onclick="grabar()">Grabar</button>
...
<!--- Ventana Adicionar -->
<div id="add_tipo" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><strong>ADICIONAR TIPO DE HORA</strong></h4>
      </div>
